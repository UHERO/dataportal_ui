...
export class HighchartComponent implements OnInit, OnChanges {
  ...
  static checkPointCount(freq, series0, point0, chart, point1?, series1?) {
    ...
    if (pointCount.length === 1) {
      chart.addSubtitle(point0, freq, point1, series1);
  ...
  drawChart(seriesData, currentFreq, portalSettings, min, max, start?, end?) {
    ...
    this.chartOptions = {
      chart: {
        ...
        events: {
          load: function () {
            /*console.log('Test load function', this.toolip);
            ...
            if (latestSeries0 > -1 && latestSeries1 > -1) {
              this.tooltip.refresh([series0.points[latestSeries0], series1.points[latestSeries1]]);
              HighchartComponent.checkPointCount(dataFreq, series0, series0.points[latestSeries0], this, series1.points[latestSeries1], series1);
            }
            // If there are no YTD values
            if (latestSeries0 > -1 && latestSeries1 === -1) {
              this.tooltip.refresh([series0.points[latestSeries0]]);
              HighchartComponent.checkPointCount(dataFreq, series0, series0.points[latestSeries0], this);
            }
            ...
            if (latestSeries0 === -1 && latestSeries1 === -1) {
              ...
              this.setTitle({ text: '<b>' + this.seriesData.seriesInfo.displayName + '</b>' });
