// Edited export-data Highcharts module to format csv export
// Allows hidden series to be exported to CSV (i.e. YTD values for non-annual series) (line 412)
// Adds data portal and series info to CSV export (see lines 376; 555)
// Original module found in node_modules/highcharts/js/modules/export-data.js
	...
	(function (Highcharts) {
		...
		Highcharts.Chart.prototype.getDataRows = function (multiLevelHeaders) {
			...
			while (i--) { // Start from end to splice in
				...
				// Add data portal & series info to the csv export
				dataRows.unshift([null], [null], [null]);
				dataRows.unshift(dpLink);
				dataRows.unshift(uhero);
				dataRows.unshift(seriesLink);
				if (sourceDetails) {
					dataRows.unshift(sourceDetails);
				}
				if (sourceLink) {
					dataRows.unshift(sourceLink);
				}
				if (sourceDescription) {
					dataRows.unshift(sourceDescription);
				}
				dataRows.unshift(seriesInfo);
